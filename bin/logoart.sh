#!/usr/bin/env bash
#
# logo.sh - render ASCII art logos based on onefetch's languages.yaml file 
# (OneFetch: https://github.com/o2sh/onefetch)
#
# Created by Claude Opus 4.8 - 22 June 2026 
#
# Usage:
#   ./logoart.sh <Language>      Show art for a language (case-insensitive)
#   ./logoart.sh --list          List all available languages
#   ./logoart.sh --random        Show a random language
#   ./logoart.sh --no-color ...  Disable ANSI colors
#   ./logoart.sh --help          Show this help
#
...
# Set default directory for logos.yaml
default_dir="$HOME/wsl-config/ascii_art"
...
if [[ -z "$yaml" ]]; then
  if   [[ -f "$default_dir/logos.yaml" ]]; then yaml="$default_dir/logos.yaml" 
  elif [[ -f "$script_dir/logos.yaml" ]]; then yaml="$script_dir/logos.yaml"
  elif [[ -f "./logos.yaml" ]];          then yaml="./logos.yaml"
  else die "logos.yaml not found; set LOGOS_YAML or use --file"; fi
fi
